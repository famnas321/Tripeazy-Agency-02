...
export const PostBookings = async (bookingData) => {
  ...
  try {
    const response = await axiosInstance.post("packages/postBookings", bookingData)
    return response.data
...
export const subscription = async (amount) => {
  console.log("subsc api hit api service page")
  try {
    const response = await axiosInstance.post("/subscription/create-payment-intent", { amount })
    return response
  } catch (error) {
    throw error.response ? error.response.data : error;
  }
...
export const addSubscriptionStatus = async (subscriptionPlan, subscriptionStatus) => {
  try {
    const response = await axiosInstance.put("/subscription/add-payment-intent", {
      subscriptionPlan,
      subscriptionStatus,
...
export const editPackage = async (updatedFields,) => {

  for (let [key, value] of updatedFields.entries()) {
    console.log(key, value, "this is updatedFields  from api");
  }

  try {
    const response = await axiosInstanceFile.put("/packages/editPackage", updatedFields)
    console.log(response.data,"this is response from auth");
